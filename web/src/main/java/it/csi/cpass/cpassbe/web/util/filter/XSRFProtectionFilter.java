 ...
 * L'idea &eacute; che il primo accesso avvenga sulla risorsa di accesso della componente SPA (es. index.html).
 * In quell'occasione il filtro non fa verifiche ma si preoccupa di creare un nuovo cookie XSRF se non presente e passarlo al client.
 * Nelle successive richieste, se la richiesta &eacute; destinata allo strato di servizi rest, viene effettuata la verifica di presenza/corrispondenza di cookie+header XSRF.
 * Questo comportamento, unito alla corretta gestione client-side del protocollo, permette di evitare il richiamo dello strato rest al di fuori del contesto corretto.
 */
public class XSRFProtectionFilter implements Filter {
	...
	/* init-params */
	/**
	 * Nome del parametro di inizializzazione che serve per disabilitare il meccanismo.
	 * Serve negli scenari di sviluppo in cui la parte client non &eacute; deployata nell'ear dei servizi
	 */
	 ...
	 *     <ul>
	 *       <li>se la request contiene una coppia di cookie e header XSRF validi => procedo, altrimenti errore.</li>
	 *     </ul>
	 *   </li>
	 * </ul>
	 */
	...
	/**
	 * &eacute; necessario fare in modo che le risorse (pagine, etc) non siano mantenute nella cache del browser per evitare che,
	 * ad un accesso in una sessione successiva, la fase di ingresso (e quindi di ricreazione del token) venga bypassata.
	 * Se ci&oacute; accadesse le richieste REST sarebbero le prime ad arrivare al server e si ricadrebbe nella casistica di token non ancora inizializzato.
	 ...
	 * @return true se il path rappresenta una richiesta alla parte REST dell'applicazione che per default deve contenere la stringa "/api/v1"
	 */
	...
	private String getActualXSRFCookie(HttpServletRequest httpReq) {
		...
		return Stream.of(cookies)
			.filter(cookie -> XSRF_COOKIE_NAME.contentEquals(cookie.getName()))
			.map(Cookie::getValue)
			.findFirst()
			.orElse(null);
	}
