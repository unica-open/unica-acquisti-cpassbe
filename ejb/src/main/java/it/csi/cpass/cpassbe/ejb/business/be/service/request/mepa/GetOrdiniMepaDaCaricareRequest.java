/*-
 * ========================LICENSE_START=================================
 * CPASS BackEnd - EJB submodule
 * %%
 * Copyright (C) 2019 - 2025 CSI Piemonte
 * %%
 * SPDX-FileCopyrightText: Copyright 2019 - 2020 | CSI Piemonte
 * SPDX-License-Identifier: EUPL-1.2
 * =========================LICENSE_END==================================
 */
package it.csi.cpass.cpassbe.ejb.business.be.service.request.mepa;

import it.csi.cpass.cpassbe.ejb.business.be.service.request.base.BasePagedRequest;

public class GetOrdiniMepaDaCaricareRequest extends BasePagedRequest {

	/**
	 * Constructor
	 */
	public GetOrdiniMepaDaCaricareRequest(Integer page, Integer size, String sort, String direction) {
		super(size, page, sort, direction);
	}

}
