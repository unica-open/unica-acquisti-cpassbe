 ...
 */
@ApplicationScoped
public class CpassTPbaInterventoCigDaoImpl extends BaseEntityDaoImpl<Integer, CpassTPbaInterventoCig> implements CpassTPbaInterventoCigDao {

	@Override
	public List<CpassTPbaInterventoCig> getCigByInterventoId(UUID interventoId) {
		final Map<String, Object> params = new HashMap<>();
		final StringBuilder jpql = new StringBuilder();
		jpql.append(" FROM CpassTPbaInterventoCig iad ");
		jpql.append(" WHERE 1=1 ");

		JpaQueryHelper.andFieldEquals(jpql, params, "iad.cpassTPbaIntervento.interventoId", "interventoId", interventoId);

		final TypedQuery<CpassTPbaInterventoCig> query = composeTypedQuery(jpql, params);
	...
	public void deleteByIdIntervento(UUID interventoId) {
		...
		sb.append("DELETE FROM CpassTPbaInterventoCig imp");
	...
	public void deleteByInterventoIdAndCig(UUID interventoId, String cig) {
		final StringBuilder sb = new StringBuilder();
		sb.append("DELETE FROM CpassTPbaInterventoCig imp");
		sb.append(" WHERE imp.cpassTPbaIntervento.interventoId = :interventoId AND imp.cig = :cig");
		final Map<String, Object> params = new HashMap<>();
		params.put("interventoId", interventoId);
		params.put("cig", cig);
		final Query query = composeQuery(sb, params);
