...
public class PostRicercaDocumentoSpesaRipartibileService extends BaseService<PostRicercaDocumentoSpesaRequest, PostRicercaDocumentoSpesaResponse> {
	 ...
	 */
	public PostRicercaDocumentoSpesaRipartibileService(ConfigurationHelper configurationHelper, ExternalHelperLookup externalHelperLookup) {
	...
	protected void execute() {
		...
		//}else {
			//checkBusinessCondition(filtro.getAnnoProtocollo()!=null && filtro.getNumeroProtocollo()!=null,MsgCpassOrd.ORDORDE0003.getError());
		//}
		ExternalServiceResolveWrapper<DocumentoSpesaHelper> handler = externalHelperLookup.lookup(DocumentoSpesaHelper.class);
		List<DocumentoSpesa> lista = invokeExternalService(handler, () -> handler.getInstance().getDocumentoSpesaRipartibile(handler.getParams(), filtro));
