...
public class PutCaricaTabelleDaTrasmettereService extends BaseCommonService<PutCaricaTabelleDaTrasmettereRequest, PutCaricaTabelleDaTrasmettereResponse> {
	...
	public PutCaricaTabelleDaTrasmettereService(ConfigurationHelper configurationHelper,
			CommonDad commonDad,
	...
	protected void execute() {
		...
		final List<Programma> listaProgrammi = programmaDad.getProgrammiConfermatiDaTrasmettere(ente.getId());
		final String progid = request.getIdProgramma();
		for(final Programma programma : listaProgrammi) {
			log.info("", "anno --> " + programma.getAnno() +" numero --> " + programma.getVersione());
			if(progid.equals("ALL") ) {
				final List<VIntervento> listInt = interventoDad.getRicercaNativaByProgrammaId(programma.getId());
				caricaTabellePerInvioAlMit(programma,listInt);
				interventoDad.flush();
			}
			if(progid.equals(programma.getId().toString())){
				final List<VIntervento> listInt = interventoDad.getRicercaNativaByProgrammaId(programma.getId());
				caricaTabellePerInvioAlMit(programma,listInt);
			}
		}
	...
	private void caricaTabellePerInvioAlMit(Programma programmaAttuale,List<VIntervento> listIntNonAnnullati) {
		// TODO Auto-generated method stub
		//List<Intervento> listIntNonAnnullati = interventoDad.getInterventoByProgrammaStato(programmaAttuale.getId(),StatoInterventiEnum.CANCELLATO.getCostante(), false);
		final List<VIntervento> listaAcqScrematiDaSoglia = scremoAcquistiInBaseAllaSoglia(programmaAttuale.getEnte().getId(),listIntNonAnnullati);
