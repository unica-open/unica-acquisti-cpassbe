...
public class PutDestinatarioOrdineService extends BaseService<PutDestinatarioOrdineRequest, PutDestinatarioOrdineResponse> {
	 ...
	 */
	public PutDestinatarioOrdineService(ConfigurationHelper configurationHelper, DestinatarioOrdineDad destinatarioDad, SettoreDad settoreDad) {
	...
	protected void execute() {
		...
		//controllo che se uno dei campi indirizzo è popolato, lo siano tutti
		...
		for(final Destinatario dest : destinatariOrdine) {
			if(dest.getSettore() != null && dest.getSettore().getCodice() != null && dest.getSettore().getCodice().equalsIgnoreCase(destinatario.getSettore().getCodice())){
				// controllo che quello trovato non sia un altro destinatario
				if(!dest.getId().equals(destinatario.getId())) {
		...
		if(destinatario.getStatoNso()!= null && destinatario.getStatoNso().getId()==null) {
			destinatario.setStatoNso(null);
		}
		destinatario = destinatarioDad.updateDestinatario(destinatario);
